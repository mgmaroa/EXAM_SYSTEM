...
from users.services import import_students_from_excel, StudentImportError
    ...
    def handle(self, *args, **options):
        ...
        try:
            with open(file_path, 'rb') as f:
                result = import_students_from_excel(f)
        except StudentImportError as exc:
            self.stderr.write(self.style.ERROR(str(exc)))
            return
        except OSError as exc:
            self.stderr.write(self.style.ERROR(f"Could not open file: {exc}"))
            return

        for row in result.created:
            self.stdout.write(self.style.SUCCESS(f"Created student: {row['student_id']} ({row['full_name']})"))

        for row in result.skipped:
            self.stdout.write(self.style.WARNING(
                f"Skipped row {row['row']} ({row['student_id'] or 'blank'}): {row['reason']}"
            ))

        self.stdout.write(self.style.SUCCESS(
            f"Import finished. Created: {result.created_count}, Skipped: {result.skipped_count}"
        ))
